...
const fastest = starts.reduce((lowestSteps, [startingX, startingY]) => {
  ...
  if (!shortestStepsMemo[`${endingX},${endingY}`]) return lowestSteps;
  return Math.min(lowestSteps, shortestStepsMemo[`${endingX},${endingY}`]);
}, Infinity);
